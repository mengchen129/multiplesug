...
;(function($, _) {
    ...
    function loadSelectOptions($select, options) {
        ...
        _.each(options || {}, function(title, key) {
            optionsHtml += '<option value="' + key + '" selected>' + title + '</option>';
        });
    ...
    $.fn.multiplesug = function(params) {
        ...
        var jsonReader = params.jsonReader || _.identity;
        ...
        _.mapObject(initData, function(val, key) {
            appendItem(key, val);
        });
        ...
        function appendItem(itemKey, itemTitle) {
            ...
            if (_.size(_.keys(selectedItems)) >= maxSelected) {
                return false;
            }
        ...
        function checkMaxSelected() {
            $sugInputWrap[(_.size(_.keys(selectedItems)) >= maxSelected) ? 'hide' : 'show']();
            $sugCopy[_.size(_.keys(selectedItems)) > 0 ? 'show' : 'hide']();
        }
        ...
        $sugInput.bind('blur', function() {
            ...
            .bind('input', _.debounce(suggestionRequest, timeout))
